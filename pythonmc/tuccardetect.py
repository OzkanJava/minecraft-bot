...
def spawnGitEnvBosalt():
    pyautogui.write('t       /is w pokerrface ',interval =sleepVal)
...
def envDolu():
    ...
    for a in range(30):
        ...
        if a == 10 :
            mouse.right_click()
        if a == 20 :
            pyautogui.moveRel(150,-35,duration= sleepVal)
            pyautogui.moveRel(150,-35,duration= sleepVal)
            time.sleep(0.5)
            mouse.right_click()
            time.sleep(0.5)
...
def checkisdisconnected():
    try:
        position = pyautogui.locateCenterOnScreen('C://Users//ozkan//Desktop//tuccar.png', confidence=0.5)
        return position
...
while True:
    print(checkCraftinIsOpen() , checkIsBigChestOpened())